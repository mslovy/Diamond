...
class RbdImageUsageCollector(diamond.collector.Collector):
    ...
    def get_default_config_help(self):
        config_help = super(RbdImageUsageCollector, self).get_default_config_help()
        config_help.update({
    ...
    def get_default_config(self):
        ...
        """
        config = super(RbdImageUsageCollector, self).get_default_config()
        config.update({
            'path':     'iostat',
            'devices':  ('rbd[0-9]+$'),
            'sector_size': 512,
    ...
    def collect(self):
        ...
        for key, info in results.iteritems():
            ...
            if not reg.match(name):
                continue
            if not rbd_to_username.has_key(info['device']):
                continue
                ...
                for unit in self.config['byte_unit']:
                    ...
                    if key.endswith('sectors'):
                        ...
                        self.MAX_VALUES[key] = diamond.convertor.binary.convert(
                            ...
                            oldUnit='byte',
                            newUnit=unit)
                    metric_name = '.'.join(["rbd", rbd_to_username[info['device']], key])
            ...
            if (metrics['io'] > 0 or self.config['send_zero']):
                for key in metrics:
                    metric_name = '.'.join(["rbd", rbd_to_username[info['device']], key]).replace(
                        '/', '_')
